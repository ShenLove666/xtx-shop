...
import { getMemeberAddressByIdAPI, postMemberAddressAPI, putMemberAddressByIdAPI } from '@/services/address'
...
// 动态设置标题
uni.setNavigationBarTitle({ title: (query.id ? '修改' : '新建') + '地址', })
...
// 获取收获地址详情页数据
const getMemeberAddressByIdData = async () => {
  if (query.id) {
    const res = await getMemeberAddressByIdAPI(query.id)
