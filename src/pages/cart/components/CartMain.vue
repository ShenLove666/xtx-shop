...
import { getMemberCartAPI, deleteMemberCartAPI, putMemeberCartBySkuIdAPI, putMemberCartSelectedAPI } from '@/services/cart'
...
const getMemeberCartData = async () => {
  const res = await getMemberCartAPI()
  cartList.value = res.result
}

// 点击删除按钮
const onDeleteCart = (stuId: string) => {
  // 弹窗二次确认
  uni.showModal({
    content: '确认要删除？',
    success: async (res) => {
      if (res.confirm) {
        // 后端删除列表
        await deleteMemberCartAPI({ ids: [stuId] })
        // 重新获取购物车数据
        getMemeberCartData()
      }
    },
...
const onChangeCount = (ev: InputNumberBoxEvent) => {
  putMemeberCartBySkuIdAPI(ev.index, { count: ev.value })
}
...
const onChangeSelected = (item: CartItem) => {
  ...
  // 后端数据更新
  putMemeberCartBySkuIdAPI(item.skuId, { selected: item.selected })
}
...
const totalPrice = computed(() => {
  // 累加每个商品的总价
  return selectedCartList.value.reduce((total, item) => total + item.nowPrice * item.count, 0).toFixed(2)
})
    ...
    <template v-if="memeberStore.profile">
      ...
      <view class="toolbar">
        ...
        <view class="button-grounp">
          <view class="button payment-button" @tap="gotoPayment" :class="{ disabled: !totalCount }"> 去结算({{
            totalCount }})
          </view>
        </view>
      </view>
