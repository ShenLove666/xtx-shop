...
<template>
  <view class="viewport">
    ...
    <!-- 推荐选项 -->
    <view class="tabs">
      <text @tap="activeIndex = index" v-for="(item, index) in subTypes" :key="item.id" class="text"
        :class="{ active: index === activeIndex }">{{ item.title
        }}</text>
    </view>
    <!-- 推荐列表 -->
    <scroll-view @scrolltolower="onScrolltolower" v-for="(item, index) in subTypes" :key="index"
      v-show="index === activeIndex" scroll-y class="scroll-view">
      <view class="goods">
