...
import { getMemberOrderPreNowAPI, getMemberOrderPreAPI, postMemberOrderAPI } from '@/services/order'
...
const getMemberOrderPreData = async () => {
  if (query.skuId && query.count) { // 如果点击的是立即购买
    const { skuId, count } = query
    const res = await getMemberOrderPreNowAPI({ skuId, count })
